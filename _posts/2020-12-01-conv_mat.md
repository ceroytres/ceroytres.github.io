...
$$ y[n] =(h * x)[n] = \sum_{m=0}^{N_y} h[n-m] x[m] $$

where $x[n]$ is the input signal of length $N_x$, $h[n]$ the filter of length $N_h$, $N_y = N_x + N_h + 1$, and $n \in \mathbb{Z}$ is the time step. 
...
Circular Convolution can be used to compute regular convolution if $x$ and $h$ are padded with zeros such that the new lengths equal $N_x + N_h - 1$. 
