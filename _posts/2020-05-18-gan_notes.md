...
$$ \text{min}_G \text{max}_D \mathbb{E}_{p(\mathbf{z})\sim Z}  [\log (1-D(G(Z))] +  \mathbb{E}_{X \sim p_{\text{data}}(\mathbf{x})}[\log D(X)]$$
...
Since we want to fool the optimal $D^\* = \text{argmax}\_D \mathbb{E}\_{Z \sim p(z)} [\log (1-D(G(Z))] +  \mathbb{E}\_{X \sim p_{\text{data}}(\mathbf{x})}[\log D(X)]$, the overall problem
becomes: 
...
GAN use a gradient ascent-descent algorithm in order to train $D$ and $G$. The algorithms is presented below:

<center>
<figure>
<img src="{{site.baseurl}}/images/post_im/gan/algo.png">
<figcaption>Training Algorithm for GANs from [1]</figcaption> 
</figure>
</center>
...
As you can see, the final discriminator converged to roughly $\frac{1}{2}$ when the generator started create samples similar to $p_{\text{data}}$ (e.g. the two histograms overlapped).
