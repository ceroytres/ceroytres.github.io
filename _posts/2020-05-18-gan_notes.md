...
The corresponding objective that the training attempts to optimize at each step of the gradient ascent-descent
...
Generator loss gradient saturates to quickly in early training. The original GAN paper [1] suggests replacing the generator's loss $\min \log(1-D(G(z)))$ with $\max \log D(G(z))$. $\max \log D(G(z))$ is refered to as the non-saturating loss.
...
Fréchet Inception Distance \cite{heusel2017gans, dowson1982frechet}:
