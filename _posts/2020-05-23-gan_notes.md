...
Generator loss gradient saturates to quickly in early training because the discriminator can easily differentiate between the real and fake samples. The original GAN paper <a href="#ref_1">[1]</a> suggests replacing the generator's loss $\min \log(1-D(G(z)))$ with $\max \log D(G(z))$. $\max \log D(G(z))$ is referred to as the non-saturating loss.
