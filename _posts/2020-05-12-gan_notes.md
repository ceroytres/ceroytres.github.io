...
An implicit model samples from a latent variable distribution $p(\mathbf{z})$ where $\mathbf{z} \in \mathcal{Z} \subseteq \mathbb{R}^m$, and uses a deterministic function $G: \mathcal{Z} \mapsto \mathcal{X}$, in order to map the latent variable $\mathbf{z}$ to $\mathbf{x} \in \mathcal{X} \subseteq \mathbb{R}^n$. The generation process is shown below as graphical model:
