...
Implict model samples from a latent variable distribution $p(z)$ where $z \in \mathcal{Z}$. Using a deterministic function $G: \mathcal{Z} \mapsto \mathcal{X}$, the latent variable $z$ is mapped to $x$. The generation process is shown below as graphical model:
