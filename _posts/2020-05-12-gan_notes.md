...
* __Zero-Sum Game__:  A game where the cost of all player equals zero. 
...
Note, $p_g$ is the same as the model distribution, $p_{\text{model}}$. A common application of implicit models is the creation of random variables (e.g. Gaussian, Laplacian, Bernoulli) from uniform distributions. For example, given two independent uniform random variables $u_1 \sim U_1, u_2 \sim U_2$ on the interval $[0,1]$ and the deterministic transform $ x = \sqrt\{-2\log u_1 \} \cos(2 \pi u_2)$ yields a unit Gaussian random variable. Typically, the function $G$ is parameterized with a set of parameters $\theta$. In the deep learning setting, $G_\theta$ is a neural network model and $\theta$ are the network parameters. When $G_\theta$ is an neural network, computing $p_g(z)$ is intractable. GAN are members of implicit models since they interact with $p_g$ through sampling and it is usually to evaluate $p_g$.
...
$$ \text{min}_G \text{max}_D \mathbb{E}_{p(\mathbf{z})\sim Z}  [\log (1-D(G(Z))] +  \mathbb{E}_{X \sim p_{data}(\mathbf{x})}[\log D(X)]$$


#### Developing the GAN Loss
Suppose you need to build a discriminative classifier $P(Y\|X)$ between real and fake examples. Let $Y \sim \text{Bern}(q)$ be a binary random variable, where $Y=1$ denotes a real sample and $Y=0$ denotes a fake or generator sample. Let's assume $p(\mathbf{x}\|y) = p_{\text{data}}(\mathbf{x})^y p_{g}(\mathbf{x})^{1-y}$. Using the cross entropy loss:
...
Since for $\mathbf{x} \sim p_{\text{data}}(\mathbf{x})$ $D(\mathbf{x}) \rightarrow 1$ and for $\mathbf{x} \sim p_{g}(\mathbf{x})$, $D(\mathbf{x}) \rightarrow 0$, $D(\mathbf{x})$ behaves exactly like our discriminative classifier. Hence we set $P(Y=1\|X)=D(\mathbf{x})$. 

$$\text{max}_{P(Y=1|X)}(1-q)\mathbb{E}_{X|Y=0} [\log (1-D(\mathbf{x})] + q \mathbb{E}_{X|Y=1}[\log D(\mathbf{x})]$$

Also, note that $p(x\|y) = p_{\text{data}}(x)^y p_{g}(x)^{1-y}$.

$$\text{max}_D(1-q)\mathbb{E}_{p_{g}} [\log (1-D(X))] + q \mathbb{E}_{x\sim p_{data}}[\log D(X)]$$ 

Since $G: \mathcal{Z} \mapsto \mathcal{X}$ and $\mathbf{z} \sim p(\mathbf{z})$:

$$\text{max}_D (1-q)\mathbb{E}_{p(z)} [\log (1-D(G(Z))] + q \mathbb{E}_{x\sim p_{data}}[\log D(X)]$$

If we select $q = 1/2$ (since we control the sampling of $Y$):

$$\text{max}_D \frac{1}{2}\mathbb{E}_{p(z)} [\log (1-D(G(Z))] + \frac{1}{2} \mathbb{E}_{x\sim p_{data}}[\log D(X)]$$

$$\text{max}_D \mathbb{E}_{p(z)} [\log (1-D(G(Z))] +  \mathbb{E}_{x\sim p_{data}}[\log D(X)]$$

In order to fool a fixed $D$, we need to find the $G$ that maximizes the loss:

$$ \text{max}_G -\mathbb{E}_{X,Y} P(Y|X) $$

$$\text{min}_G \mathbb{E}_{p(z)} [\log (1-D(G(Z))] +  \mathbb{E}_{x\sim p_{data}}[\log D(X)]$$

$$\text{min}_G \mathbb{E}_{p(z)} \log (1-D(G(Z))] $$

Since we want to fool the optimal $D^\* = \text{argmax}\_D \mathbb{E}\_{Z \sim p(z)} [\log (1-D(G(Z))] +  \mathbb{E}\_{X \sim p_{data}(\mathbf{x})}[\log D(X)]$, the overall problem
becomes: 
...
$$ \text{max}_D \int p(z) \log (1-D(G(z)) dz + \int p_{data}(x)\log D(x) dx $$

$$\text{max}_D\int p_g(x) \log (1-D(x)) + p_{data}(x)\log D(x) dx$$

$$\text{max}_D\int p_g(x) \log (1-D(x)) + p_{data}(x)\log D(x) dx \leq $$ 

$$\int \text{max}_{D(x)} [p_g(x) \log (1-D(x)) + p_{data}(x)\log D(x)] dx$$
...
$$ \int p(z) \log (1-D(G(z)) dz + \int p_{data}(x)\log D(x) dx $$

$$ F[D] = \int p_g(x) \log (1-D(x)) + p_{data}(x)\log D(x) dx $$

The functional $F$ has a stationary point at the function $D^*(x)$ that satisfies the Euler-Lagrange equation: 

$$\frac{d}{dD} [p_g(x) \log (1-D(x)) + p_{data}(x)\log D(x)]\bigg|_{D=D^*} = 0$$

$$\frac{p_{data}(x)}{D^*(x)} - \frac{p_g(x)}{1-D^*(x)}= 0$$

$$ D^*(x) = \frac{p_{data}(x)}{p_{data}(x)+p_g(x)} $$

Since $-F[D]$ is convex in $D$, $D^*$ is the global optimal discriminator. 
...
$$G^* = \text{min}_G \mathbb{E}_{p_g} [\log \frac{p_g(x)}{p_{data}(x)+p_g(x)}] + \mathbb{E}_{x\sim p_{data}}[\log \frac{p_{data}(x)}{p_{data}(x)+ p_g(x)} ]$$

$$G^* = \text{min}_G \mathbb{E}_{p_g} [\log \frac{2p_g(x)}{p_{data}(x)+p_g(x)}] + \mathbb{E}_{x\sim p_{data}}[\log \frac{2p_{data}(x)}{p_{data}(x)+ p_g(x)} ] - \log 4 $$

Using the definition of KL divergence: 

$$G^* = \min_G KL(p_g|| \frac{p_g+p_{data}}{2}) + KL(p_{data}||\frac{p_g+p_{data}}{2}) - \log 4$$

Using the definition of [Jensen Shannon Divergence](https://en.wikipedia.org/wiki/Jensen-Shannon_divergence):

$$ G^* = \min_G 2JSD(p_g || p_{data}) - \log 4 $$

Therefore, the optimal $G^\*$ requires that $p\_g = p\_{data}$ and it achieves a loss value of $-\log 4$ under the loss above. Since, the loss is convex in $p_g$ therefore $p_{data}$ is the unique global minimizer. Also, when $p_g = p_{data}$, $D^* = \frac{1}{2}$ which implies the performance of $D^*$ is equal to random guessing. 


### Training

<figure>
<img src="{{site.baseurl}}/images/post_im/gan/algo.png">
<figcaption>Training Algorithm for GANs</figcaption> 
</figure>
